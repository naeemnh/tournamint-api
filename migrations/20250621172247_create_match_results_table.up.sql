...
CREATE INDEX idx_match_results_set ON match_results(match_id, set_number) WHERE set_number IS NOT NULL;
CREATE INDEX idx_match_results_period ON match_results(match_id, period_number) WHERE period_number IS NOT NULL;
...
SELECT 
    ...
    m.match_status,
    COALESCE(SUM(CASE WHEN mr.participant1_score > mr.participant2_score THEN 1 ELSE 0 END), 0) as participant1_sets_won,
    COALESCE(SUM(CASE WHEN mr.participant2_score > mr.participant1_score THEN 1 ELSE 0 END), 0) as participant2_sets_won,
...
GROUP BY m.id, m.tournament_category_id, m.match_status;